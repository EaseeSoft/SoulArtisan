...
interface PictureResourceSelectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  assets: ParsedAsset[];
  projectId: number;
...
const PictureResourceSelectionModal: React.FC<PictureResourceSelectionModalProps> = ({
  isOpen,
  onClose,
  assets,
  projectId,
  ...
  const handleBatchCreate = async () => {
    ...
    // 必须有 scriptId 才能创建图片资源
    if (!scriptId) {
      showWarning('请先绑定剧本后再创建图片资源');
      return;
    }
    ...
    try {
      // 调用批量创建图片资源 API
      const response = await batchCreatePictureResources({
        scriptId,
      ...
      if (response.code === 200) {
        showSuccess(`成功创建 ${response.data.successCount} 个图片资源`);
      ...
      console.error('批量创建图片资源失败:', error);
      const errorMessage = error instanceof Error ? error.message : '批量创建图片资源失败';
...
export default PictureResourceSelectionModal;
